...
var iflytek = (function(document){
            ...
            "onResult": function (err, result) {
                ...
                if (err == null || err == undefined || err == 0) {
                    if (result == '' || result == null)
                        iat_result.value = "没有获取到识别结果";
                    else
                        iat_result.value = result;
                    /* 若回调的err不为空且错误码不为0，则会话失败，可提取错误码 */
                } else {
                    iat_result.value = 'error code : ' + err + ", error description : " + result;
                }
                mic_pressed = false;
                volumeEvent.stop();
            },
    ...
    var play = function() {
        if (!mic_pressed) {
            var ssb_param = {
                "grammar_list": null,
                "params": "appid=583e787a,appidkey=a757b4480da2cf76, lang = sms, acous = anhui, aue=speex-wb;-1, usr = mkchen, ssm = 1, sub = iat, net_type = wifi, rse = utf8, ent =sms16k, rst = plain, auf  = audio/L16;rate=16000, vad_enable = 1, vad_timeout = 5000, vad_speech_tail = 500, compress = igzip"
            };
            iat_result.value = '   ';
